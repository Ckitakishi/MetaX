...
struct MetadataEditTests {
    ...
    func integerValidation(current: String, replacement: String, expected: Bool) {
        ...
        #expect(viewModel.validateInput(currentText: current, range: range, replacementString: replacement, for: .iso) == expected)
    }
    ...
    func decimalValidation(current: String, replacement: String, expected: Bool) {
        let range = NSRange(location: current.count, length: 0)
        #expect(viewModel.validateInput(currentText: current, range: range, replacementString: replacement, for: .aperture) == expected)
    }
    ...
    func shutterSpeedValidation(current: String, replacement: String, expected: Bool) {
        let range = NSRange(location: current.count, length: 0)
        #expect(viewModel.validateInput(currentText: current, range: range, replacementString: replacement, for: .shutterSpeed) == expected)
    }
    ...
    func exposureBiasValidation(current: String, replacement: String, expected: Bool) {
        let range = NSRange(location: current.count, length: 0)
        #expect(viewModel.validateInput(currentText: current, range: range, replacementString: replacement, for: .exposureBias) == expected)
    }
    ...
    func globalConstraints(type: MetadataFieldType, input: String, expected: Bool) {
        let result = viewModel.validateInput(currentText: "", range: NSRange(location: 0, length: 0), replacementString: input, for: type)
